              ...
              {filters.map((filter, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Auto-enable searchable for high-cardinality fields (>100 options) */}
                    {field.type === 'select' || field.type === 'searchable' ? (
                      <SearchableSelect
                        ...
                        searchable={field.type === 'searchable' || options.length > 100}
                        multiple={field.allowMultiple}
                        placeholder={options.length > 100 ? 'Type to search...' : 'Select...'}
...
function SearchableSelect({
  ...
  const filtered = React.useMemo(() => {
    ...
    return result.slice(0, maxDisplayOptions);
  }, [options, query, maxDisplayOptions]);
  
  const hasMoreResults = React.useMemo(() => {
    if (!query) return options.length > maxDisplayOptions;
    const lower = query.toLowerCase();
    const total = options.filter(
      (o) =>
        o.label.toLowerCase().includes(lower) ||
        o.value.toLowerCase().includes(lower)
    ).length;
    return total > maxDisplayOptions;
  }, [options, query, maxDisplayOptions]);
            ...
            <div className="max-h-[200px] overflow-y-auto">
              {filtered.length === 0 ? (
                ...
                <>
                  ...
                  {hasMoreResults && (
                    <p className="text-xs text-muted-foreground p-2 text-center border-t mt-1">
