...
async function getSKUVelocityReport(
  ...
  if (fromDate) {
    orderDateFilter.gte = new Date(fromDate);
  } else {
    orderDateFilter.gte = defaultFromDate;
  }
  if (toDate) {
    orderDateFilter.lte = new Date(toDate + 'T23:59:59');
  }
...
async function getExceptionReportData(
  ...
  let items = await getExceptionReport({}, fromDate, toDate);
...
async function getCohortRetentionReportData(
  ...
  const items = await getCohortRetentionReport(fromDate, toDate);
  const totalCount = items.length;
  const start = (page - 1) * pageSize;
  const pageData = items.slice(start, start + pageSize);
...
async function getAccountPotentialReportData(
  ...
  let items = await getAccountPotentialReport(fromDate, toDate);
...
async function getRepScorecardReportData(
  ...
  let items = await getRepScorecardReport(fromDate, toDate);
...
async function getCustomerLTVReportData(
  ...
  let items = await getCustomerLTVReport(fromDate, toDate);
...
async function getFirstToSecondReportData(
  ...
  const items = await getFirstToSecondReport(fromDate, toDate);
  const totalCount = items.length;
  const start = (page - 1) * pageSize;
  const pageData = items.slice(start, start + pageSize);
  return { data: pageData, totalCount };
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (filtersParam) {
      // Try new simple format first: field:op:value,field:op:value
      // Multi-values are pipe-separated: field:eq:value1|value2
      if (!filtersParam.startsWith('%5B') && !filtersParam.startsWith('[') && filtersParam.includes(':')) {
        try {
    ...
    switch (reportType) {
      case 'category-totals':
        // Inventory snapshot - date filtering not applicable
        result = await getCategoryTotalsReport(filters, sortBy, sortDir, page, pageSize);
        break;
      case 'po-sold':
        // Shopify committed quantities - date filtering not applicable
        result = await getPOSoldReport(filters, sortBy, sortDir, page, pageSize);
        break;
      case 'sku-velocity':
        result = await getSKUVelocityReport(filters, sortBy, sortDir, page, pageSize, fromDate, toDate);
        break;
      case 'exception':
        result = await getExceptionReportData(filters, sortBy, sortDir, page, pageSize, fromDate, toDate);
        break;
      case 'cohort-retention':
        result = await getCohortRetentionReportData(page, pageSize, fromDate, toDate);
        break;
      case 'account-potential':
        result = await getAccountPotentialReportData(filters, sortBy, sortDir, page, pageSize, fromDate, toDate);
        break;
      case 'rep-scorecard':
        result = await getRepScorecardReportData(filters, sortBy, sortDir, page, pageSize, fromDate, toDate);
        break;
      case 'customer-ltv':
        result = await getCustomerLTVReportData(filters, sortBy, sortDir, page, pageSize, fromDate, toDate);
        break;
      case 'first-to-second':
        result = await getFirstToSecondReportData(page, pageSize, fromDate, toDate);
        break;
      default:
