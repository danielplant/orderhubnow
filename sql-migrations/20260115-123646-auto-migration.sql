...
-- Generated: Thu 15 Jan 2026 12:36:46 EST
...
-- ============================================
-- ORPHAN CLEANUP: CustomerOrdersItems
-- Backup orphaned records before adding FK constraint
-- ============================================

-- Create backup table for orphaned CustomerOrdersItems (if not exists)
IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name = 'CustomerOrdersItems_Orphaned_Backup')
BEGIN
    SELECT * INTO [dbo].[CustomerOrdersItems_Orphaned_Backup]
    FROM [dbo].[CustomerOrdersItems]
    WHERE [CustomerOrderID] NOT IN (SELECT [ID] FROM [dbo].[CustomerOrders]);
    
    PRINT 'Created CustomerOrdersItems_Orphaned_Backup with orphaned records';
END
ELSE
BEGIN
    -- Append to existing backup table
    INSERT INTO [dbo].[CustomerOrdersItems_Orphaned_Backup]
    SELECT * FROM [dbo].[CustomerOrdersItems]
    WHERE [CustomerOrderID] NOT IN (SELECT [ID] FROM [dbo].[CustomerOrders])
    AND [ID] NOT IN (SELECT [ID] FROM [dbo].[CustomerOrdersItems_Orphaned_Backup]);
    
    PRINT 'Appended orphaned records to existing backup table';
END

-- Delete orphaned records from main table
DELETE FROM [dbo].[CustomerOrdersItems]
WHERE [CustomerOrderID] NOT IN (SELECT [ID] FROM [dbo].[CustomerOrders]);

PRINT 'Deleted orphaned CustomerOrdersItems records';

-- ============================================
-- ORPHAN CLEANUP: AuthTokens
-- Backup orphaned records before adding FK constraint
-- ============================================

-- Create backup table for orphaned AuthTokens (if not exists)
IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name = 'AuthTokens_Orphaned_Backup')
BEGIN
    SELECT * INTO [dbo].[AuthTokens_Orphaned_Backup]
    FROM [dbo].[AuthTokens]
    WHERE [UserID] NOT IN (SELECT [ID] FROM [dbo].[Users]);
    
    PRINT 'Created AuthTokens_Orphaned_Backup with orphaned records';
END
ELSE
BEGIN
    INSERT INTO [dbo].[AuthTokens_Orphaned_Backup]
    SELECT * FROM [dbo].[AuthTokens]
    WHERE [UserID] NOT IN (SELECT [ID] FROM [dbo].[Users])
    AND [ID] NOT IN (SELECT [ID] FROM [dbo].[AuthTokens_Orphaned_Backup]);
    
    PRINT 'Appended orphaned AuthTokens to existing backup table';
END

-- Delete orphaned AuthTokens
DELETE FROM [dbo].[AuthTokens]
WHERE [UserID] NOT IN (SELECT [ID] FROM [dbo].[Users]);

PRINT 'Deleted orphaned AuthTokens records';

-- ============================================
-- Now add FK constraints (data is clean)
-- ============================================
...
IF NOT EXISTS (SELECT 1 FROM SchemaMigrations WHERE Name = '20260115-123646-auto-migration')
    INSERT INTO SchemaMigrations (Name) VALUES ('20260115-123646-auto-migration');
GO
